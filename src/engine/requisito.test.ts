...
import { ausiliaria, bando, certificazione, esecutore, fatturato, prestazione, raggruppamento, REFERENZE, requisito, servizio, soggetto } from './prova';
import { varianti } from './varianti';
import { valutaRequisito, type ContestoValutazione } from './requisito';

const CRITERIO_ISO = { tipo: 'certificazione', norme: ['ISO 9001'] } as const;
const CRITERIO_FATTURATO = { tipo: 'fatturato', ambito: { tipo: 'specifico', settore: 'dispositivi' }, periodo: { tipo: 'a_ritroso', esercizi: 3, ancoraggio: 'riferimento' }, soglia: 1_000_000 } as const;
...
/** Una lettura, nessun rinvio: le varianti sono una. */
function valutaRq(r: Requisito, c: ContestoValutazione) {
  return valutaRequisito(r, varianti(r, bando([])).varianti, c);
}
...
function contesto(soggetti: Soggetto[], membri: ReturnType<typeof esecutore>[]): ContestoValutazione {
  return {
    ...
    criterio: { dataRiferimento: '2026-09-14', dataPubblicazione: '2026-09-01', terminePresentazione: '2026-11-14' },
  };
}
...
describe('valutaRequisito — contributi', () => {
  ...
  it('un criterio di possesso non ha misurazione', () => {
    ...
    const { esito } = valutaRq(r, contesto([soggetto('s-a', [certificazione('ISO 9001', 'x')])], [esecutore('s-a', 'mandataria', { 'p-1': 1 })]));
  ...
  it('raccoglie i fatti usati per soggetto e requisito, con la scadenza', () => {
    const r = requisito('r', CRITERIO_ISO, { tipo: 'ciascun_membro' });
    const { usati } = valutaRq(r, contesto([soggetto('s-a', [certificazione('ISO 9001', 'x', '2027-01-01')])], [esecutore('s-a', 'mandataria', { 'p-1': 1 })]));
  ...
  it('raccoglie i fatti usati solo dei membri conteggiati', () => {
    ...
    const { usati } = valutaRq(r, c);
    expect(usati.map((u) => u.soggettoId)).toEqual(['s-b']);
  });
  it('salta un membro il cui soggetto non esiste', () => {
    const r = requisito('r', CRITERIO_ISO, { tipo: 'ciascun_membro' });
    const { esito } = valutaRq(r, contesto([soggetto('s-a', [certificazione('ISO 9001', 'x')])], [esecutore('s-a', 'mandataria', { 'p-1': 1 }), esecutore('s-x', 'mandante', { 'p-1': 0 })]));
  ...
  it('senza regole del motore in gioco le assunzioni sono vuote', () => {
    const r = requisito('r', CRITERIO_ISO, { tipo: 'ciascun_membro' });
    expect(valutaRq(r, contesto([soggetto('s-a', [certificazione('ISO 9001', 'x')])], [esecutore('s-a', 'mandataria', { 'p-1': 1 })])).esito.assunzioni).toEqual([]);
  });
  it('i rimedi sono vuoti: si calcolano dopo', () => {
    const r = requisito('r', CRITERIO_ISO, { tipo: 'ciascun_membro' });
    expect(valutaRq(r, contesto([soggetto('s-a')], [esecutore('s-a', 'mandataria', { 'p-1': 1 })])).esito.rimedi).toEqual([]);
  });
});
...
describe('valutaRequisito — ausiliarie', () => {
  ...
  it('su un requisito non avvalibile l’ausiliaria non conta (l’anomalia è altrove)', () => {
    ...
    expect(valutaRq(r, c).esito.stato).toBe('scoperto');
  });
